...
class NeuralRadianceField(torch.nn.Module):
    def __init__(
        ...
        self.harmonic_embed_xyz = HarmonicEmbedding(3, cfg.n_harmonic_functions_xyz)
        self.harmonic_embed_dir = HarmonicEmbedding(3, cfg.n_harmonic_functions_dir)

        embed_xyz_dim = self.harmonic_embed_xyz.output_dim
        embed_dir_dim = self.harmonic_embed_dir.output_dim

        self.MLP_skip_connection = MLPWithInputSkips(
            n_layers=cfg.n_layers_xyz,
            input_dim=embed_xyz_dim,
        ...
        if self.view_depth:
            ...
            self.mlp_features = MLPWithInputSkips(
            n_layers=cfg.append_xyz[0],
            input_dim=embed_dir_dim + cfg.n_hidden_neurons_xyz,
            output_dim=cfg.n_hidden_neurons_dir,
            skip_dim=0,
            hidden_dim=cfg.n_hidden_neurons_dir,
            input_skips=[],
            )
            self.linear_features = torch.nn.Linear(cfg.n_hidden_neurons_dir, 3)
        else:
            self.linear = torch.nn.Linear(cfg.n_hidden_neurons_xyz, 4)

    def forward(self, ray_bundle):
        embedding_xyz = self.harmonic_embed_xyz(ray_bundle.sample_points.view(-1, 3))
        y = self.MLP_skip_connection(x=embedding_xyz, z=embedding_xyz)
        if self.view_depth:
            y = self.linear(y)
            embedding_dir = self.harmonic_embed_dir(ray_bundle.directions.unsqueeze(0).repeat(
                ray_bundle.sample_points.size(1),1,1).view(-1, 3))
            features = self.mlp_features(x=torch.cat((embedding_dir, y[:, 1:]), dim=1), z=torch.cat((embedding_dir, y[:, 1:]), dim=1))
            features = self.linear_features(features)
            output = {'density': torch.nn.ReLU()(y[:, 0].reshape(-1, 1)), 'feature': torch.nn.Sigmoid()(features)}
        else:
            y = self.linear(y)
            output = {'density': torch.nn.ReLU()(y[:, 0].reshape(-1, 1)), 'feature': torch.nn.Sigmoid()(y[:, 1:])}
