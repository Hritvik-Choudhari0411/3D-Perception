...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        X_Y_grid = get_pixels_from_image(image_size, camera) # TODO (1.3): implement in ray_utils.py
        rays = get_rays_from_pixels(X_Y_grid, image_size, camera) # TODO (1.3): implement in ray_utils.py

        # TODO (1.3): Visualize xy grid using vis_grid
        if cam_idx == 0 and file_prefix == '':
            img = vis_grid(X_Y_grid, image_size)
            plt.imsave('images/section_1.3_grid.png', img)


        # TODO (1.3): Visualize rays using vis_rays
        if cam_idx == 0 and file_prefix == '':
            img = vis_rays(rays, image_size)
            plt.imsave('images/section_1.3_rays.png', img)
        
        # TODO (1.4): Implement point sampling along rays in sampler.py
        sample_points = model.sampler(rays)

        # TODO (1.4): Visualize sample points as point cloud
        if cam_idx == 0 and file_prefix == '':
            if cam_idx == 0 and file_prefix == '':
                render_points('images/section_1.4_samples_pc.png', rays.sample_points.reshape(1, -1, 3))
        ...
        h, w = image_size
        depth = np.array(output['depth'].detach().cpu().view(w, h))
        plt.imsave("images/section_1.5_depth.png", depth)
...
def train(
    ...
    for epoch in t_range:
        for iteration, batch in enumerate(train_dataloader):
            ...
            # Sample rays
            X_Y_grid = get_random_pixels_from_image(cfg.training.batch_size, image_size, camera) # TODO (2.1): implement in ray_utils.py
            rays = get_rays_from_pixels(X_Y_grid, image_size, camera)
            rgb_gt = sample_images_at_xy(image, X_Y_grid)

            # Run model forward
            output = model(rays)

            # TODO (2.2): Calculate loss
            loss = torch.nn.MSELoss()(output['feature'], rgb_gt)
...
def train_nerf(
    ...
    for epoch in range(start_epoch, cfg.training.num_epochs):
        ...
        for iteration, batch in t_range:
            ...
            # Sample rays
            X_Y_grid = get_random_pixels_from_image(
                cfg.training.batch_size, cfg.data.image_size, camera
            )
            rays = get_rays_from_pixels(
                X_Y_grid, cfg.data.image_size, camera
            )
            rgb_gt = sample_images_at_xy(image, X_Y_grid)
