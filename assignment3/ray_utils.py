...
def get_random_pixels_from_image(n_pixels, image_size, camera):
    ...
    n_samples = xy_grid.shape[0]
    xy_subgrid = xy_grid[np.random.choice(n_samples, n_pixels)].to("cuda")

    # Return
    return xy_subgrid.reshape(-1, 2)[:n_pixels]
...
def get_rays_from_pixels(xy_grid, image_size, camera):
    ...
    ndc_pts = xy_grid.cuda()

    ndc_pts = torch.cat(
        [
            ndc_pts,
            torch.ones_like(ndc_pts[..., -1:])
        ],
        dim=-1
    )

    # TODO (1.3): Use camera.unproject to get world space points from NDC space points
    world_coord_points = camera.unproject_points(ndc_pts, from_ndc=True)


    # TODO (1.3): Get ray origins from camera center
    n_samples = world_coord_points.shape[0]
    ray_origin = camera.get_camera_center().expand(n_samples, -1)

    # TODO (1.3): Get ray directions as image_plane_points - ray_origin
    ray_directions = F.normalize(world_coord_points - ray_origin)

    # Create and return RayBundle
    return RayBundle(
        ray_origin,
        ray_directions,
        torch.zeros_like(ray_origin).unsqueeze(1),
        torch.zeros_like(ray_origin).unsqueeze(1),
    )