...
class ImplicitMLPDecoder(nn.Module):
    ...
    def build_grid(self, resolution):
        ranges = [np.linspace(0.0, res - 1.0, num=res) for res in resolution]
...
class SingleViewto3D(nn.Module):
    def __init__(self, args):
        ...
        if args.type == "vox":
            ...
            vert_size = mesh_pred.verts_packed().shape[0]
    ...
    def forward(self, images, args):
        ...
        # call decoder
        if args.type == "vox":
            voxel = encoded_feat.view(-1, 64, 2, 2, 2)
            voxels_pred = self.layer1(voxel)
            return voxels_pred

        elif args.type == "point":
            point = self.decoder(encoded_feat)
            pointclouds_pred = point.view(B, self.n_point, 3)
